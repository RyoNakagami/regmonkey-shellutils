...
project: regmonkey-shellutils
---

# Branch Strategy

- **簡易版 Git Flow** を採用（`main` + `develop` の 2 本の常設ブランチを持つ点が GitHub Flow との違い）
- `release` ブランチは使わず，`develop` から直接 `main` へ PR してリリース
- `develop` への統合は Issue 規模に応じて直接 PR または Issue 統合ブランチ経由を選択

```
feature/*  ──PR──▶  develop  ──PR──▶  main  ──(CI/CD)──▶  vX.Y.Z タグ + GitHub Release
```
...
| ブランチ種別       | 命名規則                                    | 目的                                          |
| ------------ | --------------------------------------- | ------------------------------------------- |
| **本番用**      | `main`                                  | 本番デプロイ可能な安定版コード．常設ブランチ．マージ時に CI/CD でタグ + Release を自動生成 |
| **開発用**      | `develop`                               | 全機能・修正の統合先（メイン開発ブランチ）．常設ブランチ              |
...
| **テスト検証用**   | `testing/<issue番号>/<タスク>`                  | 一時的なテストコード・検証用ブランチ．検証完了後削除                  |
| **ドキュメント整備** | `document/<issue番号>/<タスク>`                  | ドキュメント・ガイド・README更新などのためのブランチ               |
...
- 統合PRを作成：`issue/0123/integration` → `develop`
...
```bash
# project.yml のバージョンを更新して develop にコミット
# version: "1.2.0"
git commit -am "RELEASE: bump version to 1.2.0"
```

`develop` → `main` への PR をマージすると CI/CD が自動で以下を実行する：

1. **`VERSION`**：`VERSION` のバージョンを読んで `vX.Y.Z` タグを打ち，GitHub Release（自動生成ノート付き）を公開

> バージョン番号の管理は手動（`main` へのPR作成前に `VERSION` を直接編集）。
...
| **中〜大規模 Issue** | `issue/<issue番号>/integration` | 複数サブタスクを統合 | サブタスクを統合して `develop` に PR | `feature` / `bugfix` をまとめてレビュー |
